...
export default function UpdateSpot() {
  ...
  const handleSubmit = async (e) => {
    ...
    !(formState.image === null) && formField.append("image", e.target.image.files[0]);
    !(formState.name === null) ? formField.append("name", e.target.name.value) : formField.append("name", spot.name);
    !(formState.lng === null) && formField.append("lng", e.target.lng.value);
    !(formState.lat === null) && formField.append("lat", e.target.lat.value);
    !(formState.address === null) && formField.append("address", e.target.address.value);
    !(formState.name === null) && formField.append("sactioned", e.target.sanctioned.value);
    !(formState.name === null) && formField.append("security", e.target.security.value);
  ...
  return (
    <div className="container">
      <h2>Add a spot</h2>
      {
        ...
        >
          {spot && (
            <Marker latitude={clickedCoordinates ?  clickedCoordinates.lat:spot.lat} longitude={clickedCoordinates ?  clickedCoordinates.lng:spot.lng} />
          )}
        </Map>
      </section>
      <form  className="addSpotForm" onSubmit={handleSubmit}>
        <label>Spot name:</label>
        <input id="name" name="name" type="text" placeholder={spot.name} onChange={handleChange} value={formState.name || formState.name===""? formState.name : spot.name}></input>
        <label>Spot image:</label>
        <input id="image" name="image" type="file" onChange={()=>setFormState({ ...formState, image: true })}></input>
        <label>Street Address (if applicable)</label>
        <input id="address" name="address" type="text" placeholder={spot.address} onChange={handleChange} value={formState.address || formState.address===""? formState.address : spot.address}></input>
